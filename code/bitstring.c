...
void InitBS() {
  ...
  for ( i=0 ; i<256 ; i++ ) {
    BitNumber[i] = 0;
    BitFirst[i] = -1;
    for ( j=0 ; j<8 ; j++ ) if (i & (1<<j)) BitNumber[i]++;
    for ( j=0 ; j<8 ; j++ )
      if ( i & (1<<j) ) {
        BitFirst[i] = j;
        break;
...
int Is0BS(BitString a) {
  int i;
  for (i=0; i<NUMBERINTS; i++) if (a[i]!=0) return(0);
  return(1);
}
...
int  EqualBS(BitString a, BitString b) {
  int i;
  for (i=0; i<NUMBERINTS; i++) if (a[i] != b[i]) return(0);
  return(1);
}

int  AllBitsSetBS(BitString x, BitString bits) {
  __m256i x_vect_1, x_vect_2, bits_vect_1, bits_vect_2;
  x_vect_1    = _mm256_loadu_si256((void *)x);
  bits_vect_1 = _mm256_loadu_si256((void *)bits);
  if (!_mm256_testc_si256(x_vect_1, bits_vect_1))
    return 0;
  x_vect_2    = _mm256_loadu_si256((void *)(x+2));
  bits_vect_2 = _mm256_loadu_si256((void *)(bits+2));
  if (!_mm256_testc_si256(x_vect_2, bits_vect_2))
    return 0;
  return 1;
}
...
/*int  AllBitsSetBS(BitString x, BitString bits) {*/
  /*int i;*/
  /*for( i = 0; i < NUMBERINTS; i++) if ( ~x[ i ] & bits[ i ] ) return 0;*/
  /*return 1;*/
/*}*/

void UnsetBS(BitString x, BitString bits) {
  int i;
  for (i=0; i<NUMBERINTS; i++) x[i] &= ~bits[i];
}

void BitAndNotBS(BitString r, BitString a, BitString b) {
  int i;
  for (i=0; i<NUMBERINTS; i++) r[i] = a[i] & ~b[i];
}
...
void BitOrAndEqBS(BitString a, BitString b, BitString c) { 
  int i;
  for (i=0; i<NUMBERINTS; i++) a[i] |= b[i] & c[i];
}
...
void BitNotBS(BitString r, BitString a) {
  int i;
  for (i=0; i<NUMBERINTS; i++) r[i] = ~a[i];
}

void BitAndEqBS(BitString a, BitString b) {
  int i;
  for (i=0; i<NUMBERINTS; i++) a[i] &= b[i];
}

void BitNandEqBS(BitString a, BitString b) { 
  int i;
  for (i=0; i<NUMBERINTS; i++) a[i] = ~(a[i] & b[i]);
}
...
int  LogAndBS (BitString a, BitString b) {
  int i;
  for (i=0; i<NUMBERINTS; i++) if ((a[i] & b[i]) != 0) return(1);
  return(0);
}
...
int  LogAndAndNotBS(BitString a, BitString b, BitString c) {
  int i;
  for (i=0; i<NUMBERINTS; i++) if (((a[i] & b[i]) & ~c[i]) != 0) return(1);
  return(0);
}
...
int  LogNorAndNotBS (BitString r, BitString a, BitString b) {
  int i;
  for (i=0; i<NUMBERINTS; i++) if (((r[i]^a[i])&~b[i])!=0) return(1);
  return(0);
}
