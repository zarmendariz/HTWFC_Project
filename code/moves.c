  ...
  for (i=0; i<maze->number_stones; i++) {
    ...
    for (dir=NORTH; dir<=WEST; dir++) {
      if ((IsBitSetBS( maze->reach,pos+DirToDiff[dir]))
        ...
        SETMOVE(moves[number_moves], 1, pos, pos-DirToDiff[dir], pos+DirToDiff[dir]);
        if (!DeadLock(maze,moves+number_moves)) {
          if (SubMacro(maze,moves,&number_moves)) goto END;
          number_moves++;
...
int NewMoveOrdering(int depth, int number_moves)
{
  ...
  if (number_moves>1) {
    lmove = depth ? &(IdaInfo->IdaArray[depth-1].currentmove) : &DummyMove;
    ...
    for (i=0; i<number_moves; i++) {
      ...
      /* Use the closest entrance to the goal area the
       * stone is in (if) as goalpos */
      goalpos = maze->goals[maze->lbtable[maze->PHYSstone[m->from]].goalidx].loc;
      ...
      if (m->macro_id != 4) diff = m->value - StoneDistManpos(maze,m->to,goalpos,m->from);
      else diff = m->value;
      if (diff>0) {
        if (lmove->to == m->from) m->value -= ENDPATH;
      }
      else {
        m->value -= diff*100;
      }
...
{
  if (number_moves>1) {
    My_qsort(&(IdaInfo->IdaArray[depth].moves), number_moves, sizeof(MOVE), compare_val);
  }
  return(number_moves);
}
...
int DistToGoal(MAZE *maze, PHYSID start, PHYSID goal, PHYSID *last_over) {
  static PHYSID stack[XSIZE*YSIZE], from[XSIZE*YSIZE], dist[XSIZE*YSIZE];
  ...
  while (next_out < next_in) {
    ...
    for (dir=NORTH; dir<=WEST; dir++) {
      if (IsBitSetBS(maze->reach,pos+DirToDiff[dir])) s_visited[dir][pos] = dis;
    }
    for (dir=NORTH; dir<=WEST; dir++) {
...
void PushesMoves(MAZE *maze, PHYSID start, PHYSID goal, int *pushes, int *moves,
...
void PushesMoves2(MAZE *maze, PHYSID start, PHYSID goal, int *pushes, int *moves,
    BitString stone_squares, BitString man_squares)
{
  static PHYSID      stack[XSIZE*YSIZE];
  static PHYSID      from[XSIZE*YSIZE];
...
int ValidSolution(MAZE *maze, MOVE *solution) {
  ...
  while (!ISDUMMYMOVE(solution[i])) {
    p = m = 0;
    PushesMoves(maze, solution[i].from, solution[i].to, &p, &m, stone, man);
    number_pushes += p;
    number_moves  += m;
