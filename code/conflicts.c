...
void AddPenalties(CONFLICTS *c)
{
  ...
  c->pen=My_realloc(c->pen, sizeof(PENALTY)*(c->array_size_pen+CONFLICT_INC));
...
void InsertPenalty(CONFLICTS *c, int i, int p)
{
  ...
  memmove(c->pen+i+1, c->pen+i, sizeof(PENALTY)*(c->number_penalties-i));
...
PENALTY *FindPenalty(CONFLICTS *c, int penalty)
{
  ...
  i=0;
  while ((i < c->number_penalties) && (penalty < c->pen[i].penalty)) i++;
...
int RemoveDuplicates( CONFLICTS *c, int penalty, BitString pattern, BitString no_reach )
{
  int peni, coni;
...
void PrintConflicts(MAZE *maze, CONFLICTS *c)
{
  ...
  for (peni=0; peni<c->number_deadtested; peni++) {
    ...
    PrintBit3Maze(maze, c->deadtested[peni].stones, c->deadtested[peni].relevant, 0);
  }
  Mprintf( 0, "Number of pentested: %d\n", c->number_pentested);
  for (peni=0; peni<c->number_pentested; peni++) {
    maze->manpos = c->pentested[peni].manpos;
    PrintBit3Maze(maze, c->pentested[peni].stones, c->pentested[peni].relevant, 0);
  }
...
void AddTestedPen(CONFLICTS *c, BitString relevant, BitString stones, PHYSID manpos,
    PHYSID stonepos, int goodtested)
{
  TESTED *pen;
 ...
 */
int WasTestedPen(CONFLICTS *c, BitString stones, BitString reach, PHYSID stonepos)
{
  int i;
  ...
  for (i=0; i<c->number_pentested; i++) {
    if ((IsBitSetBS(reach,c->pentested[i].manpos))
        ...
        &&(!LogAndAndNotBS(stones, c->pentested[i].relevant, c->pentested[i].stones))) {
      return(i+1);
    }
  }
  return(0);
...
int WasTestedDead(CONFLICTS *c, BitString stones, BitString reach, PHYSID stonepos)
{
  int i;
  ...
  for (i=0; i<c->number_deadtested; i++) {
    if ((stonepos==c->deadtested[i].stonepos)
        ...
        &&(!LogAndAndNotBS(stones, c->deadtested[i].relevant, c->deadtested[i].stones))) {
      return(i+1);
    }
  }
  return(0);
  ...
  for (peni=0; peni<c->number_penalties; peni++) {
    for (coni=0; coni<c->pen[peni].number_conflicts; coni++) {
      ...
      if (IsBitSetBS(stones,c->pen[peni].cflts[coni].onestone)
          && !IsBitSetBS( c->pen[ peni ].cflts[ coni ].no_reach, manpos )
          && AllBitsSetBS(stones, c->pen[peni].cflts[coni].conflict) ) {
        ...
        PrintBit3Maze(IdaInfo->IdaMaze,
            ...
            c->pen[peni].cflts[coni].no_reach,0);
        if (penalty>=ENDPATH) {
          SR(Debug(CFLTS_PPLEVEL, 0, "GetPenalty: Deadlock found\n"));
          goto END_GETPEN;
        }
      }
    }
...
void FindTransitiveClosure(int pattern, BitType *patterns, BitType *conflict_table)
{
 ...
 */
int Maximize(BitType open, BitType *used, BitType *conflict_table, int *penalties)
{
  ...
  while (pattern != -1) {
    ...
    penalty = Maximize(new_open,&tmp_used,conflict_table,penalties) + penalties[pattern];
...
int  GetPenalty(MAZE *maze, int targetpen)
{
  ...
  char      conflict_count[BitSize]; /* How many patterns conflict with this one */
  BitType   conflict_table[BitSize]; /* Bit pattern representing with which*/
  BitString stones;                  /* this pattern conflicts.*/
  BitString used_stones; /* For overestimation */
  PHYSID    pos;
  ...
  for (peni=0; peni<c->number_penalties; peni++) {
    for (coni=0; coni<c->pen[peni].number_conflicts; coni++) {
      if (IsBitSetBS( stones, c->pen[peni].cflts[coni].onestone )
          && !IsBitSetBS( c->pen[ peni ].cflts[ coni ].no_reach, maze->manpos )
          && AllBitsSetBS(stones, c->pen[peni].cflts[coni].conflict) ) {
        if (c->pen[peni].penalty>=targetpen) {
          c->pen[peni].cflts[coni].n_used++;
          c->pen[peni].cflts[coni].t_used = total_node_count;
          SR(Debug(CFLTS_PPLEVEL, 0, "GetPenalty: early pen found: %i (s:%i)\n",
        ...
        match_count++;
        BitOrEqBS(used_stones, c->pen[peni].cflts[coni].conflict);
  ...
  if (match_count == 0) {
    ...
    for (peni=0; peni<match_count; peni++) {
      ...
      for (coni=peni+1; coni<match_count; coni++) {
        ...
        if (LogAndBS(cflts[peni]->conflict, cflts[coni]->conflict)) {
          /* They conflict */
    ...
    while (BitsNot0(patterns_left)) {
      ...
      if (conflict_count[pattern] == 0) {
        ...
        tmp_used = 0;
        total_penalty += Maximize(bpatterns, &tmp_used, conflict_table, penalties);
...
int  GetPenaltyImp1(MAZE *maze, int targetpen)
{
  ...
  BitSet0(patterns_all);
  Set0BS(used_stones);
  #pragma omp parallel shared(early_return, c, num_threads)
  {
    thrd_num = omp_get_thread_num();
    if (thrd_num == 0) num_threads = omp_get_num_threads();
    #pragma omp barrier
  for (peni=thrd_num; peni<c->number_penalties-num_threads; peni+=num_threads) {
    for (coni=0; coni<c->pen[peni].number_conflicts; coni++) {
  ...
  if (early_return == true) return maze->pen;
...
ENDLOOP:
  ...
  if (total_penalty >= targetpen) {
    ...
    penalty = 0;
    #pragma omp parallel shared(penalty)
    {
      thrd_num = omp_get_thread_num();
    for (peni=thrd_num; peni<maze->number_stones-num_threads; peni+=num_threads) {
      pos = maze->stones[peni].loc;
      if (IsBitSetBS(used_stones,pos)) {
        this_pen = 0;
        for (coni=0; coni<match_count; coni++) {
